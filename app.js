// App v6: hardcoded proxies, no sample fallback
  ...
  <section id="panel-${k}" class="game-panel hidden">
    <div class="hero" style="background-image:${gradients[k]}">
      <div class="flex items-center justify-between gap-4">
        <div><h2 class="text-2xl font-semibold">${g.name}</h2><p class="opacity-80 text-sm">Community sentiment & latest posts.</p></div>
...
function showPanel(key){
  ...
  if(CFG.games[key]) renderNewsFor(key);
}
...
function renderNewsFor(key){
  const g=CFG.games[key]; const t=document.getElementById(`news-${key}`); if(!g||!t) return;
  t.innerHTML=(g.news||[]).map(n=>`<a href="${n.href}" target="_blank" rel="noopener" class="card block hover:ring-2 hover:ring-sky-400"><div class="flex items-center justify-between gap-4"><div><h4 class="font-medium">${n.title}</h4><p class="text-xs opacity-70 mt-1">${n.src||''}</p></div><span class="pill">Open ↗</span></div></a>`).join('');
}
...
async function loadAll(){
  ...
  for(const [key,g] of Object.entries(CFG.games)){
    const redditPosts = await callProxy(CFG.proxies.reddit, { subreddits:g.subreddits||[], limit:25 });
    const twitterPosts = await callProxy(CFG.proxies.twitter, { handles:g.twitter?.handles||[], hashtags:g.twitter?.hashtags||[], game:key, limit:50 });
    const threadsPosts = await callProxy(CFG.proxies.threads, { handles:g.threads?.handles||[], game:key, limit:40 });
